...
/**
  ...
  */
void get_path(char **token, char **arr, char **file, int *filelen)
{
	int tkin = 0, arrlen, ind1, ind2 = 0;
...
char *_which(char *file)
{
	...
	int filelen = 0, tkin = 0, arrlen, ind1, ind2 = 0, state;
	...
	else
	{
		...
		while (token != NULL)
		{
			get_path(&token, &arr, &file, &filelen);
			if ((stat(arr, &st) == 0))
			{
				state = 1;
				break;
